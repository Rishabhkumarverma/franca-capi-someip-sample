...
set(capi-core-gen PATH=$ENV{PATH} ${COMMONAPI_PATH}/commonapi_core_generator/commonapi-core-generator-linux-x86_64)
set(capi-someip-gen PATH=$ENV{PATH} ${COMMONAPI_PATH}/commonapi_someip_generator/commonapi-someip-generator-linux-x86_64)
#
...
function(append_capi_interface_hdr Target Path Version)
    set(SOURCE_LINK ${Target}_SOURCE_CAPI_HDR_FILES)
    list(APPEND ${SOURCE_LINK}
        ${CMAKE_CURRENT_BINARY_DIR}/v${Version}/${Path}.hpp
    )
    list(APPEND ${SOURCE_LINK}
        ${CMAKE_CURRENT_BINARY_DIR}/v${Version}/${Path}Stub.hpp
    )
    list(APPEND ${SOURCE_LINK}
        ${CMAKE_CURRENT_BINARY_DIR}/v${Version}/${Path}StubDefault.hpp
    )
    list(APPEND ${SOURCE_LINK}
        ${CMAKE_CURRENT_BINARY_DIR}/v${Version}/${Path}ProxyBase.hpp
    )
     list(APPEND ${SOURCE_LINK}
        ${CMAKE_CURRENT_BINARY_DIR}/v${Version}/${Path}Proxy.hpp
    )
    set(${SOURCE_LINK} ${${SOURCE_LINK}} PARENT_SCOPE)
endfunction(append_capi_interface_hdr)
...
macro(add_capi_interfaces Target Interface...)
    ...
    foreach(iface ${Interfaces})
        ...
        append_capi_interface_hdr(${Target} ${INTERFACE_PATH} ${INTERFACE_VERSION})
        get_fidl_file(${iface} FIDL_FILE)
        append_fidl_file(${Target} ${FIDL_PATH}/${FIDL_FILE})
    endforeach(iface)
endmacro(add_capi_interfaces)

function(add_capi_old_library Target)
...
endfunction(add_capi_old_library)
...
function(add_capi_library Target)
    ...
    set(CAPI_HEADERS ${Target}_HEADER)
    add_custom_target(${CAPI_HEADERS} ALL DEPENDS ${${SOURCE_LINK}})
    add_library(${Target}  INTERFACE)
    add_dependencies(${Target} ${CAPI_HEADERS})
